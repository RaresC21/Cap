...
function spo_loss(B_new, X, c, oracle; z_star=[])
	...
	if z_star == []
        ...
        (z_worst, w_worst) = oracle_dataset(-c, oracle)
	end
	...
	for i = 1:n
		...
		optimal_value = dot(c[:,i], w_star[:,i])

		spo_loss_cur = dot(c[:,i], w_oracle) - optimal_value
        spo_loss_worst =  dot(c[:,i], w_worst[:,i]) - optimal_value
		spo_star += z_star[i]
		spo_sum = spo_sum + (spo_loss_cur) / optimal_value
        println(" LOSSES, ", spo_loss_cur, " ", spo_loss_worst, " ", (spo_loss_cur) / (spo_loss_worst))

		adam_loss_cur = dot(c[:,i], w_oracle) - z_star[i]
		total +=  z_star[i]
		adam_loss = adam_loss_cur + adam_loss

	end
    println("Final Loss:", spo_sum / n, " Adam Loss: ", adam_loss/total)
	spo_loss_avg = spo_sum/n
	return spo_loss_avg, adam_loss / total
end
...
#     end
    ...
    dot_prods = (B_true * X_observed) .^ 2
#     println(size(B_true * X_observed), " ", size(dot_prods))
	# dot_prods = abs.(B_true*X_observed)
...
function generate_poly_kernel_data_simple(B_true, n, polykernel_degree, noise_half_width)
	...
	return generate_poly_kernel_data(B_true, n, polykernel_degree;
        ...
        normalize_c = true,
		inner_constant = 1, outer_constant = 0)
end
